...
import { TunaiCard, TunaiInput, TunaiButton, TunaiBadge, TunaiTextarea, TunaiSelect, TunaiToggle } from '../components/TunaiComponents';
...
export const VoucherGenerator: React.FC = () => {
  ...
  useEffect(() => {
      ...
      window.addEventListener('tunai-fill-voucher' as any, handleFillForm as any);
      window.addEventListener('tunai-add-item' as any, handleAddItem as any);
      window.addEventListener('tunai-download-pdf' as any, handleDownload as any);
      
      return () => {
          window.removeEventListener('tunai-fill-voucher' as any, handleFillForm as any);
          window.removeEventListener('tunai-add-item' as any, handleAddItem as any);
          window.removeEventListener('tunai-download-pdf' as any, handleDownload as any);
      };
  }, [autoFilledFields]);
  ...
  return (
    <div className="space-y-6 md:space-y-8 relative max-w-7xl mx-auto pb-12 p-2 md:p-0">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start gap-4">
        ...
        <div className="w-full lg:w-96 bg-white rounded-2xl p-4 shadow-lg border border-purple-100 flex flex-col gap-3 relative overflow-hidden group">
            ...
            <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden z-10">
                <div 
                    className={`h-full transition-all duration-1000 progress-bar-fill ${progress === 100 ? 'bg-green-500' : 'bg-purple-500'}`} 
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-end w-full">
            <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleReceiptScan} title="Upload Receipt" />
            <input type="file" ref={batchInputRef} className="hidden" accept="image/*" onChange={handleBatchScan} multiple title="Batch Upload" />

            <TunaiButton onClick={() => fileInputRef.current?.click()} disabled={scanning} className="flex-1 sm:flex-none flex items-center justify-center gap-2 text-sm">
                <Upload size={16} /> {t('uploadReceipt')}
            </TunaiButton>
            <TunaiButton onClick={handleReadAloud} disabled={playingAudio} className="flex-1 sm:flex-none flex items-center justify-center gap-2 text-sm">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8">
        ...
        <div className="xl:col-span-4 h-full">
            <TunaiCard className="h-full">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-gray-600 uppercase tracking-wider">{t('companyInfo')}</h3>
                    <input type="file" ref={companyDocInputRef} onChange={handleCompanyScan} className="hidden" accept="image/*" title="Upload Company Document" />
                ...
                <div className="space-y-4">
                    <div className="flex flex-col items-center justify-center mb-6">
                         <div className="relative w-28 h-28 group">
                            ...
                            {companyLogo && <button onClick={handleRemoveLogo} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1.5 shadow-md" title="Remove Logo"><X size={12} strokeWidth={3} /></button>}
                            <input ref={logoInputRef} type="file" accept="image/*" onChange={handleLogoUpload} className="hidden" title="Upload Logo" />
                         </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <TunaiInput name="companyRegNo" value={companyRegNo} onChange={(e) => { setCompanyRegNo(e.target.value); handleFieldChange('companyRegNo'); }} placeholder={PLACEHOLDERS.companyRegNo} className={getAutoFillClass('companyRegNo')} />
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <TunaiTextarea name="companyAddress" value={companyAddress} onChange={(e) => { setCompanyAddress(e.target.value); handleFieldChange('companyAddress'); }} placeholder={PLACEHOLDERS.companyAddress} className={`resize-none ${getAutoFillClass('companyAddress')}`} />
                    ...
                    <div className="grid grid-cols-1 gap-4">
                         <div>
                            ...
                            <TunaiInput name="companyTel" value={companyTel} onChange={(e) => { setCompanyTel(e.target.value); handleFieldChange('companyTel'); }} placeholder={PLACEHOLDERS.companyTel} className={getAutoFillClass('companyTel')} />
                         </div>
                         <div>
                            <label className="block text-xs text-gray-500 mb-1">{t('email')}</label>
                            <TunaiInput name="companyEmail" value={companyEmail} onChange={(e) => { setCompanyEmail(e.target.value); handleFieldChange('companyEmail'); }} placeholder={PLACEHOLDERS.companyEmail} className={getAutoFillClass('companyEmail')} />
                         </div>
                    </div>
                </div>
            </TunaiCard>
        ...
        <div className="xl:col-span-8 space-y-6">
            <TunaiCard title={t('voucherDetails')}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div>
                        <label className="block text-sm text-gray-500 mb-2">{t('voucherNo')}</label>
                        <TunaiInput name="voucherNo" value={voucherNo} onChange={(e) => setVoucherNo(e.target.value)} placeholder={PLACEHOLDERS.voucherNo} />
                    </div>
                    <div>
                        <label className="block text-sm text-gray-500 mb-2">{t('voucherDate')}</label>
                        <div className="relative">
                             <TunaiInput name="date" type="date" value={voucherDate} onChange={(e) => { setVoucherDate(e.target.value); handleFieldChange('voucherDate'); }} className={getAutoFillClass('voucherDate')} />
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <TunaiSelect name="category" value={category} onChange={(e) => setCategory(e.target.value)} className={getAutoFillClass('category')}>
                    ...
                    <div className="md:col-span-2 pt-4 border-t border-gray-200/50">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div className="relative">
                                    <TunaiInput name="payee" value={payee} onChange={(e) => { setPayee(e.target.value); handleFieldChange('payee'); }} placeholder={PLACEHOLDERS.payee} className={getAutoFillClass('payee')} />
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <TunaiInput name="payeeIc" value={payeeIc} onChange={(e) => { setPayeeIc(e.target.value); handleFieldChange('payeeIc'); }} placeholder={PLACEHOLDERS.payeeIc} className={getAutoFillClass('payeeIc')} />
                                </div>
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm text-gray-500 mb-2">Payee Address</label>
                                <TunaiInput name="payeeAddress" value={payeeAddress} onChange={(e) => setPayeeAddress(e.target.value)} placeholder={PLACEHOLDERS.payeeAddress} />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-500 mb-2">{t('paymentMethod')}</label>
                                <TunaiSelect name="paymentMethod" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)}>
                             ...
                             <div>
                                ...
                                <TunaiInput name="bankName" value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder={PLACEHOLDERS.bankName} disabled={paymentMethod === 'Cash'} className={paymentMethod === 'Cash' ? 'opacity-50' : ''} />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm text-gray-500 mb-2">Payment Reference / Cheque No</label>
                                <TunaiInput name="paymentRef" value={paymentRef} onChange={(e) => setPaymentRef(e.target.value)} placeholder={PLACEHOLDERS.paymentRef} disabled={paymentMethod === 'Cash'} className={paymentMethod === 'Cash' ? 'opacity-50' : ''} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 pt-4 border-t border-gray-200/50">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <TunaiInput name="preparedBy" value={preparedBy} onChange={(e) => setPreparedBy(e.target.value)} placeholder="Name" />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-500 mb-2">Approved By</label>
                                <TunaiInput name="approvedBy" value={approvedBy} onChange={(e) => setApprovedBy(e.target.value)} placeholder="Name" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2">
                        <div className="flex justify-between items-center mb-2">
                            ...
                            <div className="flex gap-2">
                                <TunaiButton onClick={toggleDictation} active={isDictating} className="!py-1.5 !px-3 text-xs">{isDictating ? t('stop') : t('dictate')}</TunaiButton>
                                <TunaiButton onClick={handleAISummary} disabled={loadingAI} className="!py-1.5 !px-3 text-xs">{t('aiSummarize')}</TunaiButton>
                            </div>
                        </div>
                        <div className="relative">
                            <TunaiTextarea name="description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder={PLACEHOLDERS.description} rows={2} className={getAutoFillClass('description')} />
            ...
            <TunaiCard title={t('taxCompliance')} className="mt-6">
                    ...
                    <div className="space-y-4">
                        <div>
                         ...
                         <div>
                            ...
                            <TunaiInput name="taxCode" value={taxCode} onChange={(e) => { setTaxCode(e.target.value); handleFieldChange('taxCode'); }} placeholder={PLACEHOLDERS.taxCode} className={getAutoFillClass('taxCode')} />
                        </div>
                    </div>
                 </div>
            </TunaiCard>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="xl:col-span-2">
            <TunaiCard title={t('lineItems')} className="h-full">
                <div className="space-y-4">
                    {items.map((item, index) => (
                        <div key={item.id} className="group p-4 rounded-xl bg-white/30 border border-white/40 shadow-sm flex flex-col md:flex-row gap-4 items-center">
                            ...
                            <div className="w-full md:flex-1">
                                ...
                                <TunaiInput name={`item-${item.id}-description`} value={item.description} onChange={(e) => updateItem(item.id, 'description', e.target.value)} className={WHITE_INPUT_THEME} />
                            </div>
                            <div className="w-full md:w-40 relative">
                                <label className="md:hidden text-xs text-gray-500 mb-1 block">Amount</label>
                                <TunaiInput name={`item-${item.id}-amount`} type="number" value={item.amount} onChange={(e) => updateItem(item.id, 'amount', e.target.value)} className={`text-right ${getAutoFillClass(`item-${item.id}-amount`)}`} />
                            </div>
                            <button onClick={() => deleteItem(item.id)} className="text-gray-400 hover:text-red-600 p-2" title="Delete Item"><Trash2 size={20} /></button>
                        </div>
                    ))}
                    <TunaiButton onClick={addItem} className="text-sm w-full md:w-auto"><Plus size={16} className="inline mr-2" /> {t('addItem')}</TunaiButton>
                </div>
            </TunaiCard>
        </div>
        ...
        <div className="xl:col-span-1">
            <TunaiCard title={t('summary')} className="h-full flex flex-col justify-between">
                <div className="flex flex-col items-center justify-center flex-1 py-6 space-y-4">
                    ...
                    <TunaiBadge color="text-blue-600 bg-blue-100/50 px-4 py-1">Draft</TunaiBadge>
                </div>
                
                <div className="pt-6 border-t border-gray-200/50 space-y-3">
                    {/* Action Buttons */}
                    <div className="grid grid-cols-2 gap-3">
                         <TunaiButton onClick={handleExportExcel} className="w-full text-green-700 text-xs py-2 flex items-center justify-center gap-1 bg-green-50 hover:bg-green-100 border border-green-200">
                             <FileSpreadsheet size={14} /> Excel
                         </TunaiButton>
                         <TunaiButton onClick={handleExportWord} className="w-full text-blue-800 text-xs py-2 flex items-center justify-center gap-1 bg-blue-50 hover:bg-blue-100 border border-blue-200">
                             <FileText size={14} /> Word
                         </TunaiButton>
                    </div>

                    <TunaiButton onClick={handlePreviewPDF} className="w-full text-gray-600 text-sm py-3 flex items-center justify-center gap-2 bg-gray-200 hover:bg-gray-300">
                        <Eye size={16} /> {t('previewPdf')}
                    </TunaiButton>
                    <TunaiButton onClick={handleDownloadPDF} className="w-full text-red-600 text-sm py-3 flex items-center justify-center gap-2">
                        <Download size={16} /> {t('downloadPdf')}
                    </TunaiButton>
                    <div className="flex gap-3">
                        <TunaiButton onClick={() => executeSaveVoucher('DRAFT')} disabled={saving} className="flex-1 text-gray-600 text-sm">Draft</TunaiButton>
                        <TunaiButton onClick={handleSaveClick} disabled={saving} className="flex-1 text-blue-600 text-sm font-bold">{t('save')}</TunaiButton>
                    </div>
                </div>
            </TunaiCard>
        </div>
      ...
      <TunaiCard title="Lost Receipt Details (Optional)">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               <div>
                    ...
                    <div className="relative">
                        <TunaiInput name="originalDate" type="date" value={originalDate} onChange={(e) => { setOriginalDate(e.target.value); handleFieldChange('originalDate'); }} className={getAutoFillClass('originalDate')} />
               ...
               <div>
                    ...
                    <TunaiInput name="evidenceType" placeholder={PLACEHOLDERS.evidenceType} value={evidenceType} onChange={(e) => setEvidenceType(e.target.value)} className={WHITE_INPUT_THEME} />
               </div>
               <div>
                    <label className="block text-sm text-gray-500 mb-2">Evidence Reference</label>
                    <TunaiInput name="evidenceRef" placeholder={PLACEHOLDERS.evidenceRef} value={evidenceRef} onChange={(e) => setEvidenceRef(e.target.value)} className={WHITE_INPUT_THEME} />
               </div>
               <div className="md:col-span-2 lg:col-span-3">
                    <label className="block text-sm text-gray-500 mb-2">Reason for Lost Receipt</label>
                    <TunaiTextarea name="lostReason" placeholder={PLACEHOLDERS.lostReason} value={lostReason} onChange={(e) => setLostReason(e.target.value)} rows={2} className={`resize-none ${WHITE_INPUT_THEME}`} />
               </div>
          </div>
      </TunaiCard>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
             ...
             <div className="bg-white rounded-2xl w-full max-w-4xl h-[90vh] flex flex-col relative z-10 shadow-2xl">
                 <div className="flex justify-between items-center p-4 border-b">
                     ...
                     <button onClick={() => setShowPdfPreview(false)} title="Close Preview"><X size={24} /></button>
                 </div>
                 ...
                 <div className="p-4 border-t flex justify-end gap-3 bg-white rounded-b-2xl">
                     <TunaiButton onClick={() => setShowPdfPreview(false)} className="text-sm">Close</TunaiButton>
                     <TunaiButton onClick={handleDownloadPDF} className="text-sm text-green-600">Download PDF</TunaiButton>
                 </div>
             </div>
          </div>
      )}
      ...
      {errorModal.show && (
            ...
            <TunaiCard className="w-full max-w-sm relative z-10 shadow-2xl border-2 border-red-100 text-center bg-[#e0e5ec]">
                    ...
                    <div className="flex gap-3 justify-center">
                        <TunaiButton onClick={() => setErrorModal({ ...errorModal, show: false })} className="text-sm">Dismiss</TunaiButton>
                    </div>
            </TunaiCard>
            </div>
      )}
      ...
      {showBatchModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
             ...
             <TunaiCard className="w-full max-w-4xl h-[80vh] flex flex-col relative z-10 shadow-2xl p-0 overflow-hidden bg-[#e0e5ec]">
                 <div className="flex justify-between items-center p-4 border-b border-gray-300/50">
                     ...
                     <button onClick={() => !isBatchProcessing && setShowBatchModal(false)} title="Close Batch Queue"><X size={24} /></button>
                 </div>
                             ...
                             <div key={item.id} className="bg-[#e0e5ec] rounded-xl p-3 shadow-sm flex gap-4 items-center">
                                 ...
                                 <div className="flex-1">
                                     ...
                                     {item.status === 'completed' && (
                                         <TunaiButton onClick={() => loadFromBatch(item)} className="!py-1 !px-2 text-xs mt-2">Load Data</TunaiButton>
                                     )}
                                 </div>
                             </div>
                         ))}
      ...
      {showOCRConfirm && extractedData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            ...
            <TunaiCard className="w-full max-w-lg relative z-10 shadow-2xl border-2 border-purple-100 max-h-[90vh] overflow-y-auto bg-[#e0e5ec]">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold text-gray-800">Verify Extraction</h3>
                    <button onClick={() => setShowOCRConfirm(false)} title="Close"><X size={18} /></button>
                </div>
                
                <div className="space-y-4 mb-8">
                     <TunaiInput value={extractedData.payeeName || ''} onChange={(e) => handleExtractedDataChange('payeeName', e.target.value)} placeholder="Merchant Name" />
                     <TunaiInput type="date" value={extractedData.date || ''} onChange={(e) => handleExtractedDataChange('date', e.target.value)} />
                     <TunaiInput type="number" value={extractedData.totalAmount || ''} onChange={(e) => handleExtractedDataChange('totalAmount', parseFloat(e.target.value))} />
                     
                     <div className="border-t pt-4 mt-4">
                        <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Bill To</h4>
                        <TunaiInput value={extractedData.companyName || ''} onChange={(e) => handleExtractedDataChange('companyName', e.target.value)} placeholder="Company Name" />
                     </div>
                </div>
                
                <div className="flex gap-4 justify-end">
                    <TunaiButton onClick={() => setShowOCRConfirm(false)}>Cancel</TunaiButton>
                    <TunaiButton onClick={handleApplyOCRConfirm} className="text-white bg-purple-600">Apply Details</TunaiButton>
                </div>
            </TunaiCard>
        </div>
      )}
      ...
      {showConfirmDialog && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
            ...
            <TunaiCard className="w-full max-w-md relative z-10 shadow-2xl border-2 border-blue-100 text-center bg-[#e0e5ec]">
                 ...
                 <div className="flex gap-3 justify-center">
                     <TunaiButton onClick={() => setShowConfirmDialog(false)}>Back</TunaiButton>
                     <TunaiButton onClick={() => executeSaveVoucher('COMPLETED')} className="text-white bg-blue-600">Confirm & Save</TunaiButton>
                 </div>
            </TunaiCard>
          </div>
      )}
